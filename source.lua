...
pcall(function() _G.Dp638MqzuBrUX7Awf5J9eTJyb4C7a0eKG2ff6 = true end)

COREGUI = game:GetService("CoreGui")
if not game:IsLoaded() then
    local notLoaded = Instance.new("Message")
    notLoaded.Parent = COREGUI
    notLoaded.Text = 'IY is waiting for the game to load'
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "IY FE v" .. currentVersion
...
Credits.Text = "Edge // Zwolf // Moon // Toon // ttwiz_z"
...
About.TextSize = 14
About.Text = "Plugins are .iy files and should be located in the 'workspace' folder of your Software."
...
function toClipboard(String)
    if clipBoard then
        ...
        notify('Clipboard',"Your Software doesn't have the ability to use the clipboard")
    end
end
...
eventEditor = (function()
    ...
    main.Name = randomString()
    local mainFrame = main:WaitForChild("Content", math.huge)
    local eventList = mainFrame:WaitForChild("List", math.huge)
    local eventListHolder = eventList:WaitForChild("Holder", math.huge)
    local cmdTemplate = mainFrame:WaitForChild("CmdTemplate", math.huge)
    local eventTemplate = mainFrame:WaitForChild("EventTemplate", math.huge)
    local settingsFrame = mainFrame:WaitForChild("Settings", math.huge):WaitForChild("Slider", math.huge)
    local settingsTemplates = mainFrame.Settings:WaitForChild("Templates", math.huge)
    local settingsList = settingsFrame:WaitForChild("List", math.huge):WaitForChild("Holder", math.huge)
    ...
    settingsFrame:WaitForChild("Close", math.huge).MouseButton1Click:Connect(function()
        ...
        if pcall(function() readfile("IY_FE.iy") end) then
            if readfile("IY_FE.iy") ~= nil then
                local success, response = pcall(function()
                    ...
                    if json.keepIY ~= nil then Keep_IY_FE = json.keepIY else Keep_IY_FE = true end
            ...
            if pcall(function() readfile("IY_FE.iy") end) then
                ...
                Directions.Text = "There was a problem writing a save file to your PC.\n\nPlease contact the developer/support team for your Software and tell them writefile is not working.\n\nYour settings, keybinds, waypoints, and aliases will not save if you continue.\n\nThings to try:\n> Make sure a 'workspace' folder is located in the same folder as your Software\n> If your Software is inside of a zip/rar file, extract it.\n> Rejoin the game and try again or restart your PC and try again."
...
function updatesaves()
    if nosaves == false and writefileSoftware() then
        local update = {
            prefix = prefix;
            StayOpen = StayOpen;
            keepIY = Keep_IY_FE;
...
if not writefileSoftware() then
    notify('Saves','Your Software does not support read/write file. Your settings will not save.')
end
...
SaveChatlogs.MouseButton1Down:Connect(function()
    if writefileSoftware() then
        if #scroll_2:GetChildren() > 0 then
            ...
            local writelogs = '-- IY Chat logs for "'..placeName..'"\n'
            for _, child in pairs(scroll_2:GetChildren()) do
                writelogs = writelogs..'\n'..child.Text
            end
            ...
            nameFile()
            notify('Chat Logs','Saved chat logs to the workspace folder within your Software folder.')
        end
    else
        notify('Chat Logs','Your Software does not support write file. You cannot save chat logs.')
    end
end)
...
local chatbox
task.spawn(function()
    local success, result = pcall(function() chatbox = game.WaitForChild(PlayerGui, "Chat", math.huge).Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar end)
    if success then
        ...
        PlayerGui:WaitForChild("Chat", math.huge).Frame.ChatBarParentFrame.ChildAdded:Connect(function(newbar)
            wait()
            if newbar:FindFirstChild('BoxFrame') then
                chatbox = PlayerGui:WaitForChild("Chat", math.huge).Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the IY support server.'}
CMDs[#CMDs + 1] = {NAME = 'console', DESC = 'Loads old Roblox console'}
CMDs[#CMDs + 1] = {NAME = 'explorer / dex', DESC = 'Opens DEX by ttwiz_z'}
...
function refresh(plr)
    ...
    respawn(plr)
    task.spawn(function()
        plr.CharacterAdded:Wait():WaitForChild("Humanoid", math.huge).RootPart.CFrame, workspace.CurrentCamera.CFrame = pos, wait() and pos1
        refreshCmd = false
    end)
end
            ...
            if cmd then
                ...
                if infTimes then
                    while lastBreakTime < cmdStartTime do
                        ...
                        if not success and _G.IY_DEBUG_Dp638MqzuBrUX7Awf5J9eTJyb4C7a0eKG2ff6 then
...
Plugins.MouseButton1Click:Connect(function()
    if writefileSoftware() then
        ...
        notify('Incompatible Software','Your Software is unable to use plugins (missing read/writefile)')
    end
end)
...
addcmd('keepiy', {}, function(args, speaker)
    if queueteleport then
        ...
        notify('Incompatible Software','Your Software does not support this command (missing queue_on_teleport)')
    end
end)
...
addcmd('exit',{},function(args, speaker)
    if COREGUI ~= PlayerGui then
        ...
        Players.LocalPlayer:Kick("Shutdown")
    end
end)
...
local mobilefly = function(speaker, vfly)
    ...
    local controlModule = require(speaker.PlayerScripts:WaitForChild("PlayerModule", math.huge):WaitForChild("ControlModule", math.huge))
    local bv = Instance.new("BodyVelocity")
...
addcmd('savegame',{'saveplace'},function(args, speaker)
    if saveinstance then
        ...
        notify('Game Saved','Saved place to the workspace folder within your Software folder.')
    else
        notify('Incompatible Software','Your Software does not support this command (missing saveinstance)')
    end
end)
...
addcmd('clientantikick',{'antikick'},function(args, speaker)
    if not hookmetamethod then 
        return notify('Incompatible Software','Your Software does not support this command (missing hookmetamethod)')
    end
    local LocalPlayer = Players.LocalPlayer
    local oldhmmi
    local oldhmmnc
...
addcmd('setfpscap', {'fpscap', 'maxfps'}, function(args, speaker)
    if setfpscap and type(setfpscap) == "function" then
        ...
        return notify('Incompatible Software', 'Your Software does not support this command (missing setfpscap)')
    end
end)
...
addcmd('noclipcam', {'nccam'}, function(args, speaker)
    ...
    if not sc or not getgc or not gc then
        return notify('Incompatible Software', 'Your Software does not support this command (missing setconstant or getconstants or getgc)')
    end
...
addcmd('2022materials',{'use2022materials'},function(args, speaker)
    if sethidden then
        ...
        notify('Incompatible Software','Your Software does not support this command (missing sethiddenproperty)')
    end
end)
...
addcmd('autojump',{'ajump'},function(args, speaker)
    ...
    HumanModCons.ajCA = (HumanModCons.ajCA and HumanModCons.ajCA:Disconnect() and false) or speaker.CharacterAdded:Connect(function(nChar)
        Char, Human = nChar, nChar:WaitForChild("Humanoid", math.huge)
        autoJump()
        HumanModCons.ajLoop = (HumanModCons.ajLoop and HumanModCons.ajLoop:Disconnect() and false) or RunService.RenderStepped:Connect(autoJump)
    end)
end)
...
addcmd('refreshanimations', {'refreshanimation', 'refreshanims', 'refreshanim'}, function(args, speaker)
    ...
    local Human = Char and Char:WaitForChild('Humanoid', math.huge)
    local Animate = Char and Char:WaitForChild('Animate', math.huge)
...
addcmd('spoofspeed',{'spoofws','spoofwalkspeed'},function(args, speaker)
    if args[1] and isNumber(args[1]) then
        if hookmetamethod then
            ...
            notify('Incompatible Software','Your Software does not support this command (missing hookmetamethod)')
        end
    end
end)
...
addcmd('explorer', {'dex'}, function(args, speaker)
    notify('Loading', 'Hold on a sec')
    loadstring(game:HttpGet("https://raw.githubusercontent.com/TT0-0wiZZ/backup/main/dex.lua"))()
end)
...
addcmd('audiologger',{'alogger'},function(args, speaker)
    notify("Loading",'Hold on a sec')
    loadstring(game:HttpGet(('https://raw.githubusercontent.com/TT0-0wiZZ/backup/main/audiologger.lua'),true))()
end)
...
addcmd('fireclickdetectors',{'firecd','firecds'}, function(args, speaker)
    if fireclickdetector then
        ...
        notify("Incompatible Software", "Your Software does not support this command (missing fireclickdetector)")
    end
end)
...
addcmd('fireproximityprompts',{'firepp'},function(args, speaker)
    if fireclickdetector then
        ...
        notify("Incompatible Software", "Your Software does not support this command (missing fireproximityprompt)")
    end
end)
...
addcmd('clearhats',{'cleanhats'},function(args, speaker)
    if firetouchinterest then
        ...
        notify("Incompatible Software","Your Software does not support this command (missing firetouchinterest)")
    end
end)
...
addcmd('dupetools', {'clonetools'}, function(args, speaker)
    ...
    for i = 1, LOOP_NUM do
        local Human = speaker.Character:WaitForChild("Humanoid", math.huge)
        ...
        speaker.CharacterAdded:Wait():WaitForChild("Humanoid", math.huge).Parent:MoveTo(LOOP_NUM == i and OrigPos or TempPos, wait(.1))
...
addcmd('givetool', {'givetools'}, function(args, speaker)
    ...
    speaker:ClearCharacterAppearance()
    task.spawn(function()
        speaker.CharacterAdded:Wait():WaitForChild('Humanoid', math.huge).RootPart.CFrame = wait() and hrppos
    end)
...
function kill(speaker,target,fast)
    if tools(speaker) then
        if target ~= nil then
            ...
            speaker.CharacterAdded:Wait():WaitForChild("HumanoidRootPart", math.huge).CFrame = NormPos
        end
    else
        notify('Tool Required','You need to have an item in your inventory to use this command')
    end
...
addcmd('autoclick',{},function(args, speaker)
    if mouse1press and mouse1release then
        ...
        notify('Auto Clicker',"Your Software doesn't have the ability to use the autoclick")
    end
end)
    ...
    if player:IsInGroup(RolewatchData.Group) then
        if tostring(player:GetRoleInGroup(RolewatchData.Group)):lower() == RolewatchData.Role:lower() then
            if RolewatchData.Leave == true then
                Players.LocalPlayer:Kick("Rolewatch | Player \"" .. tostring(player.Name) .. "\" has joined with the Role \"" .. RolewatchData.Role .. "\"")
            else
...
addcmd('clearnilinstances',{'nonilinstances','cni'},function(args, speaker)
    if getnilinstances then
        ...
        notify('Incompatible Software','Your Software does not support this command (missing getnilinstances)')
    end
end)
...
addcmd('autokeypress',{'keypress'},function(args, speaker)
    if keypress and keyrelease and args[1] then
        ...
        notify('Auto Key Press',"Your Software doesn't have the ability to use auto key press")
    end
end)
...
task.spawn(function()
    ...
    if success then
        if currentVersion ~= latestVersionInfo.Version then
            notify('Outdated','Get the new version at github.com/ttwizz/infiniteyield')
        end
...
task.spawn(function()
    ...
    minimizeHolder()
    if IsOnMobile then notify("Unstable Device", "On mobile, IY may have issues or features that are not functioning correctly.") end
end)
